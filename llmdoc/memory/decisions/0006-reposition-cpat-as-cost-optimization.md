...
> **2026-06-25 范式修正（重要）**：本决策的结论**仅在「非受限窗口」（百万真实窗口）语境下成立**。
> 按 CAT 论文（arXiv:2512.22087）的「受限窗口」范式重做实验后，全量双扫（硬窗口 32K/200K × 三臂）**推翻了「非质量提升」这一半**：受限窗口下 CPAT 相对 ReAct 是**质量的决定性提升**（0%→100%），相对被动 threshold 在大窗口下是**巨大成本优化**（省 83%）。前五组之所以测不出，是因为百万窗口让 ReAct 永不溢出——这是范式错误而非 CPAT 无用。本决策据此**降级为「非受限窗口语境下的局部结论」**，完整修正与证据见 `memory/decisions/0007-bounded-window-cpat-value.md`。下文保留作历史记录。

- **状态**：已采纳（项目叙事级重定位,由五组 CPAT vs ReAct 对照实验累积证据得出）；**2026-06-25 由 0007 部分推翻，限定在非受限窗口语境**
- **日期**：2026-06-14
...
- **restore 的价值是明确的开放问题**:验证它需要「信息真正超过模型物理窗口、ReAct 必然失败」的场景,而这在 V4 Pro + 我们可及的 benchmark 上无法构造(见 0005 决定性证据)。不要把 restore 描述为已验证的优势。**2026-06-25 补充**:双扫已构造出 ReAct 因受限窗口提前终止的场景（见 0007），但 restore 仍为 0——信息源是静态文件、可重读，agent 永远选 re-read。restore 的独立价值仍未验证。
