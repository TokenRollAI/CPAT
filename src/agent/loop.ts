...
- Treat the context as working memory you govern. Prefer small targeted patches over letting the runtime force-compress at critical pressure.
- pressure=soft: consider patching now.
- pressure=must_act: before continuing the task you MUST either call context_update, or include the literal phrase "no_context_update_needed" in your reply with a one-line justification.
...
export async function runAgent(opts: {
  ...
  const tools: ToolDefinition[] = [...taskToolDefs, artifactGetTool, contextUpdateTool];

  runtime.ingestSystem(SYSTEM_PROMPT);
  ...
  for (let turn = 1; turn <= config.maxTurns; turn++) {
    ...
    if (resp.message.tool_calls?.length) {
      ...
      turnLogs.push(log);
      onTurn?.(log);
      return { answer, turns: turn, turnLogs, metrics: collectMetrics(runtime), runtime };
    }
    turnLogs.push(log);
    onTurn?.(log);
  }
...
function collectMetrics(runtime: ContextRuntime): RunMetrics {
